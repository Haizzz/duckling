...
class TaskDetail {
                ...
                <div class="py-1">
                  ...
                  ${
                    canWatch
                      ? `
                    <button 
                      onclick="TaskDetailInstance.watchTask()"
        ...
        <div class="space-y-6">
          ...
          <div>
            <div class="flex justify-between items-center mb-3">
              ...
              <a 
                href="/api/tasks/${task.id}/log-file" 
                target="_blank"
  ...
  async watchTask() {
    const success = await Utils.watchTask(this.taskId, {
      hideDropdown: () => this.hideTaskDropdown(),
    });

    // Re-start log refresh since the task is now in awaiting-review
    if (success) {
      this.startLogRefresh();
    }
  }
