...
class Dashboard {
  ...
  renderTaskCard(task) {
    ...
    const repositoryInfo = repository ?
      `<span class="text-sm text-gray-600 break-words">${repository.name}</span>` :
      `<span class="text-sm text-gray-500 font-mono break-words">${this.escapeHtml(task.repository_path || 'Unknown')}</span>`;
    ...
    return `
      <div class="task-card bg-white border border-gray-200 rounded-lg p-6 hover:shadow-sm transition-shadow ${borderClass}" data-task-id="${task.id}">
        ...
        <div class="flex justify-between items-start mb-4">
          <a href="task-detail.html?id=${task.id}" class="text-lg font-medium text-gray-900 flex-1 mr-4 hover:text-blue-600 hover:underline break-words">${this.escapeHtml(summary)}</a>
          ${statusBadge}
        </div>
  ...
  getBranchLink(branchName, repositoryPath) {
    ...
    if (repository && repository.owner && repository.name) {
      ...
      return `<a href="${githubUrl}" target="_blank" class="text-blue-600 text-sm hover:text-blue-800 underline font-mono break-words">${this.escapeHtml(branchName)}</a>`;
    }
    // Fallback to plain text if repository info is not available
    return `<span class="text-sm text-gray-600 font-mono break-words">${this.escapeHtml(branchName)}</span>`;
  }
