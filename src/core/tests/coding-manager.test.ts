...
describe('CodingManager', () => {
  ...
  describe('generateCode', () => {
    it('calls amp CLI with API key environment variable and task ID when amp tool configured', async () => {
      ...
      const execSpy = jest.spyOn(execModule, 'execCommandWithInput').mockResolvedValue({
        stdout: 'Generated code',
        stderr: '',
        exitCode: 0
      });

      const result = await codingManager.generateCode('amp', 'Create a function', { 
        taskId: 123 
      });
    ...
    it('calls codex CLI with OpenAI API key and task prompt when openai tool configured', async () => {
      ...
      const whichSpy = jest.spyOn(execModule, 'execCommand')
        .mockResolvedValueOnce({ stdout: '/usr/local/bin/codex', stderr: '', exitCode: 0 })
        .mockResolvedValueOnce({ stdout: 'AI generated code', stderr: '', exitCode: 0 });

      const result = await codingManager.generateCode('openai', 'Create a class', { 
        taskId: 456 
      });

      expect(result).toBe('AI generated code');
      expect(mockDb.getSetting).toHaveBeenCalledWith('openaiApiKey');
      expect(whichSpy).toHaveBeenCalledWith('which', ['codex'], { taskId: 456 });
    ...
    it('throws error with specific message when API key not found in database', async () => {
      ...
      await expect(codingManager.generateCode('amp', 'task', { taskId: 123 }))
  ...
  describe('requestFixes', () => {
    it('calls generateCode with original task and concatenated error messages', async () => {
      const generateCodeSpy = jest.spyOn(codingManager, 'generateCode').mockResolvedValue('Fixed code');
